...
def approx_average_is_average(hand):
    ...
    realAverage = sum(hand) / len(hand)
    FLAverage = (hand[0]+hand[-1]) / 2
    if len(hand) % 2:
        mediana = hand[len(hand) // 2]
    else:
        mediana = hand[len(hand)] + hand [len(hand + 1)]

    if realAverage == FLAverage or realAverage == mediana:
        return True
    return False
...
def average_even_is_average_odd(hand):
    ...
    evenAverage = sum(even) / len(even)
    oddAvergae = sum(odd) / len(odd)

    return evenAverage == oddAvergae
